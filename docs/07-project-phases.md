...
**Objective:** Implement the approved v1 customer payment policy: Razorpay full payment and secured COD with a mandatory 50% Razorpay advance. Retain Manual Payment and direct UPI as non-storefront operational capabilities.
**Deliverables:** Razorpay integration (order creation, payment capture, webhook handling with idempotency); secured COD flow (50% Razorpay advance, 50% due on delivery); Manual Bank Transfer operational flow (screenshot upload via Phase 4 media pipeline, "Payment Verification Pending" order state, admin Approve/Reject UI with reason capture); direct UPI operational flow; Payment History model/API; paid/outstanding-balance tracking.
...
**Acceptance Criteria:** Razorpay always creates a gateway order for the full server-calculated checkout total; COD always creates a gateway order for exactly 50% of the server-calculated total and remains Pending Payment until capture; successful COD advance capture transitions the order to COD Confirmed (or Pre-Order Confirmed for pre-order carts); webhook processing is idempotent; paid and outstanding balances remain accurate.
...
**Deliverables:** Pre-order configuration fields on Product/Variant (enable, start/end date, expected dispatch/delivery date, quantity cap); payment-method-driven checkout using the Phase 10 policy (Razorpay full payment or secured COD with 50% Razorpay advance); pre-order quantity-cap enforcement and auto-close; Production Tracker model (9 stages) linked to pre-order line items; customer-facing tracker UI on Order detail; admin bulk production-stage update; stage-transition notifications (ties to Phase 21 notifications).
...
**Acceptance Criteria:** A pre-order product correctly enforces date window and quantity cap (including concurrent-order race test); Razorpay/full and secured-COD/50%-advance totals are server-authoritative; production stage updates cascade correctly to customer-visible tracker and trigger notifications; admin can batch-update stage across multiple pre-orders in one action.
